...
//  Prova
//
//  Created by Mario Brischetto on 03/11/15.
//  Copyright (c) 2015 Mario Brischetto. All rights reserved.
//

#ifndef __Prova__low_layer__
#define __Prova__low_layer__
...
//PROCESS_NAME(low_layer_analyzer);
...
void accepted_table_remove(accepted_table *t, address address){
	...
	if (addressCmp(address, e->address)==0){
		...
		*/
		...
		//printf("accepted_element - reset (1)\n");
		return;
	} else {
		...
		for (i=2; i<t->len; i++){	//0=SRC, 1=BROADCAST
			if (addressCmp(address, e->address)==0){
				...
				free(e);
				t->len --;
				//printf("accepted_element - eliminato (2)\n");
				return;
			}
			e_before = e;
			e = e->next;
	...
	//printf("accepted_element - problema\n");
}
/*************************************************************************/
...
void accepted_table_init(){
	if (accepted_table_init_var == 0){
		...
		//printf("LOW_LAYER - acceptedTable init\n");
	}
}
/*************************************************************************/
...
static void recv_packet(struct unicast_conn *c, const linkaddr_t *from, uint8_t data[]) {
    ...
    switch (h.type){
    case DATA:
    	if (accepted_table_search(acceptedTable, h.next_hop)==1) {
    		if (accepted_table_search(acceptedTable, h.dst)==1) {
    ...
    case BEACON:
    	if (accepted_table_search(acceptedTable, h.dst)==1){
    		...
    		//printf("LOW_LAYER - BEACON - error : dst = other\n");
    	}
    	break;
    case REPORT:
    	if (accepted_table_search(acceptedTable, h.next_hop)==1) {
    		if (accepted_table_search(acceptedTable, h.dst)==1) {
    			//printf("LOW_LAYER - REPORT : dst = myself - sono il sink, inoltra al controller?\n");
    ...
    case OPEN_PATH:
    	if (accepted_table_search(acceptedTable, h.next_hop)==1
    			&& accepted_table_search(acceptedTable, h.dst)==1){
    ...
    case (REQUEST+CONFIG):
		if (accepted_table_search(acceptedTable, h.next_hop)==1) {
			//if (accepted_table_search(acceptedTable, h.dst)==1) {
			if (addressCmp(sink_address, h.next_hop) == 0) {
...
#endif /* defined(__Prova__low_layer__) */
